(function(){
  ...
  function loadData() {
    ...
    partsData = {
      ratchets: PARTS_DATA.gears.map(function(g) { return {code: g.id, name: g.name, type: g.type, best_for: g.note, source: g.note, image: '/parts/gear_' + g.id.replace('-','_') + '.png'}; }),
      bits: PARTS_DATA.axes.map(function(a) { return {code: a.id, name: a.name, type: a.type, best_for: a.note, source: a.note, image: '/parts/axis_' + a.id + '.png'}; }),
      recommended_combos: []
    };
